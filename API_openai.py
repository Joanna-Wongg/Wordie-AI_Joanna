...
import requests
import json
from openai import OpenAIError #ADDED THIS
...
class API_Call_openai():
    ...
    def __init__(self, agent=None):
        ...
        if agent is None:
            ...
            #ADDING DEBUGGING CHANGE
            if not isinstance(self.agent_data.get("system_prompt"), (str, list)):
                print("[ERROR] system_prompt in agent JSON is not a string or valid message content.")
...
#---------------CHANGES--------------
        if not any(msg["role"] == "system" for msg in conversation):
            system_prompt = self.agent_data.get("system_prompt") or self.agent_data.get("PrePrompt", "")
            ...
            if isinstance(system_prompt, dict) and "role" in system_prompt and "content" in system_prompt:
                ...
                print(f"[WARNING] Invalid system prompt format: {type(system_prompt)}. Using empty string.")
        ...
        try:
            ...
            except OpenAIError as e: #except openai.error.AuthenticationError as e: CHANGED
